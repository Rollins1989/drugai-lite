...
# Target-specific activity models are optional. They are created by
# train_target_activity.py. The first bundled target is EGFR/CHEMBL203.
TARGET_MODELS = {}
...
def target_activity_prediction(smiles: str, target_name: str) -> dict:
    ...
    if model_bundle is None:
        ...
        raise HTTPException(
            status_code=404,
            detail=f"No trained target model named '{target_name}'. Available: {available}. "
                   "Run train_target_activity.py first.",
        )
    ...
    confidence = "HIGH" if disagreement < 0.35 else ("MODERATE" if disagreement < 0.75 else "LOW")
    ...
    return {
        ...
        "active_at_pIC50_6": bool(pred >= 6.0),
        "training_evaluation": model_bundle["metrics"],
        "note": "pIC50 is a model prediction, not experimental activity.",
    }
...
def model_cards():
    return {"version": app.version, "metrics": METRICS, "features": [
        ...
        "Murcko scaffold", "applicability-domain signal", "descriptor-importance explanation", "random + scaffold evaluation", "target-specific pIC50 prediction",
    ]}
...
def activity_targets():
    return {
        "targets": [
            {
                "name": name,
                "target_chembl_id": bundle["metrics"].get("target_chembl_id"),
                "evaluation": bundle["metrics"].get("model", {}),
            }
            for name, bundle in sorted(TARGET_MODELS.items())
        ]
    }
